# Krill behaviour project
# Testing water column velocity properties:
# After reading Hudson et al. (2022), who showed that DVM can result in increased
# local retention, I reconsidered the observation that krill spent more time close to the
# surface when chlorophyll a concentrations were low.
# If DVM behaviour also relates to advection, spending more time at the surface implies
# increased spatial displacement due to the comparatively stronger surface currents.
# Thus, this could also be a way for krill to relocate when local feedings conditions 
# become unfavourable.
# To get an overview about this hypothesis, I downloaded advection model data for our on a
# investigated area during the same time period on a 0.083x0.083 grid from the Copernicus 
# Marine Service (https://doi.org/10.48670/moi-00016).
# In the following, I analyze some of the properties of the water column, velocity distributions
# and see whether and how these related to krill behaviour.
# ------------------------------------------------------------------------------------------------- #
# Load libraries
library(terra)
library(scico)
